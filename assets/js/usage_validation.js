function updateAvailableStock() {
  ...
  document.getElementById(
    "stock-info"
  ).textContent = `Available Stock: ${stock.toLocaleString()} sheets`;
...
function calculateReamsLive() {
  ...
  const sets =
    parseFloat(document.querySelector('[name="number_of_sets"]').value) || 0;
  const copies =
    parseFloat(document.querySelector('[name="copies_per_set"]').value) || 0;
  ...
  const output = `Estimated Reams per Product: ${reamsPerProduct.toFixed(
    2
  )} reams`;
